...
const CARTE_PER_PAGINA_RACCOGLITORE = 16;
const CARTE_PER_META_PAGINA_RACCOGLITORE = 8;
...
function renderContenutoFatiche() {
  ...
  modalCard?.classList.add("fatiche-fullscreen");
...
function htmlHubFatiche() {
  return `
    <div style="display:flex; flex-direction:column; gap:14px; padding:20px; align-items:center; justify-content:center; height:100%;">
      ...
      <button type="button" id="hub-btn-augia" class="events-btn events-btn-main" style="width:100%; max-width:360px; padding:18px; font-size:1rem;">
        🐄 Le Stalle di Augia
      </button>
  ...
  return `
    <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; height:100%; gap:14px; padding:14px;">
      <div style="background:rgba(15,10,5,0.6); border-radius:10px; padding:14px; color:#e0d5c1; font-size:0.85rem; max-width:340px; text-align:center;">
        Ripulisci le stalle allineando almeno 3 creature uguali in riga o colonna: spariscono e ti fanno guadagnare punti. Hai <b>10 mosse</b> a disposizione — più punti fai, più Dracme guadagni!
      </div>
...
// ===== Swipe verso il basso per uscire dalle sezioni Fatiche a schermo intero (nessun tasto visibile) =====

document.getElementById("battle-report-content")?.addEventListener("touchstart", (e) => {
  if (e.touches.length !== 1) return;
  faticheSwipeStartY = e.touches[0].clientY;
  faticheSwipeStartX = e.touches[0].clientX;
}, { passive: true });

document.getElementById("battle-report-content")?.addEventListener("touchend", (e) => {
  if (faticheSwipeStartY === null) return;

  const modalCard = document.querySelector("#battle-result-modal .modal-card");
  if (!modalCard || !modalCard.classList.contains("fatiche-fullscreen")) {
    faticheSwipeStartY = null;
    faticheSwipeStartX = null;
    return;
  }

  const deltaY = e.changedTouches[0].clientY - faticheSwipeStartY;
  const deltaX = Math.abs(e.changedTouches[0].clientX - faticheSwipeStartX);
  faticheSwipeStartY = null;
  faticheSwipeStartX = null;

  if (deltaY > 90 && deltaX < 60) {
    sezioneFaticheCorrente = "hub";
    renderContenutoFatiche();
  }
}, { passive: true });
