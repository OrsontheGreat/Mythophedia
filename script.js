...
function apriPannelloDentroIlMito() {
  ...
  const sezioniHtml = cultureOrdinate.map(cultura => {
    const nomiOrdinati = gruppi[cultura].sort((a, b) => a.localeCompare(b, "it"));
    ...
    return `
      <div class="mito-gruppo-cultura" data-cultura-mito="${cultura.toLowerCase()}">
        <div class="mito-titolo-cultura">${cultura} <span class="mito-conteggio-cultura">(${nomiOrdinati.length})</span></div>
        <div class="mito-schede-cultura">${schedeHtml}</div>
      </div>`;
  }).join("");

  document.getElementById("battle-report-content").innerHTML = `
    <div style="padding:10px 20px; display:flex; flex-direction:column; height:100%; min-height:0;">
      <input type="text" id="mito-search-input" placeholder="Cerca una creatura o una mitologia..." 
      ...
      <div id="mito-lista-schede" style="flex:1; min-height:0; overflow-y:auto; display:flex; flex-direction:column; gap:16px;">
        ${sezioniHtml}
      </div>
    </div>`;
  ...
  document.getElementById("mito-search-input").addEventListener("input", (e) => {
    ...
    document.querySelectorAll(".mito-gruppo-cultura").forEach(gruppo => {
      const cultura = gruppo.dataset.culturaMito;
      let almenoUnaVisibile = false;
      gruppo.querySelectorAll(".mito-scheda").forEach(scheda => {
        const match = scheda.dataset.nomeMito.includes(filtro) || cultura.includes(filtro);
        scheda.style.display = match ? "" : "none";
        if (match) almenoUnaVisibile = true;
      });
      gruppo.style.display = almenoUnaVisibile ? "" : "none";
    });
  });
