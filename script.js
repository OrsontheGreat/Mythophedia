...
const CARTE_FISSE = [
  ...
  { nome: "Pegaso", cultura: "Greca", tratti: ["volo"], immagine: "img/carte/pegaso.jpg", livello: 1, statisticheFisse: { ferocia: 0.8, balzo: 3.0, corazza: 3.9, istinto: 0.3 } },
...
function controllaFineSettimanaMondo(chiaveMappa, vecchiaMappa, callback) {
  ...
  dbFirebase.ref("mondi_meta/" + chiaveMappa).once("value").then((snapMeta) => {
    ...
    const cappelloLivelloCartaPerMondo = { p: 1, i: 2, e: 3, c: 4, l: 6 };
    ...
    if (posizioneMia >= 0 && classifica[posizioneMia].count > 0) {
      ...
      let dracmeBase, ambraBase, cartaBonus;

      if (posizioneMia === 0) { dracmeBase = 300; ambraBase = 2; cartaBonus = 2; }

      else if (posizioneMia === 1) { dracmeBase = 150; ambraBase = 1; cartaBonus = 1; }

      else if (posizioneMia === 2) { dracmeBase = 75; ambraBase = 0; cartaBonus = 1; }

      else { dracmeBase = 30; ambraBase = 0; cartaBonus = 0; }
      ...
      for (let k = 0; k < cartaBonus; k++) {
        ...
        let nuovaCarta = estraiCartaPerLivello(livelloCartaPremio);
