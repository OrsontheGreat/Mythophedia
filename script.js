...
function renderizzaSelezioneSottomondi() {
  ...
  STRUTTURA_SOTTOMONDI.forEach(sub => {
    ...
    btn.addEventListener("click", () => {
      ...
      const mapModalCard = document.querySelector("#worlds-modal .map-modal-card");
      if (mapModalCard) {
        const sfondo = SOTTOMONDO_SFONDI[sub.id];
        mapModalCard.style.backgroundImage = sfondo ? `linear-gradient(rgba(10,8,5,0.62), rgba(10,8,5,0.62)), url('${sfondo}')` : "none";
        mapModalCard.style.backgroundSize = "cover";
        mapModalCard.style.backgroundPosition = "center";
      }
...
document.getElementById("close-sottomondi-modal").addEventListener("click", () => { 
  ...
  if (worldsTitle.innerText !== "Seleziona Mondo") {
