...
const LORE_CARTE = {
  ...
  "Nidhogg": "Drago scandinavo che rode incessantemente le radici di Yggdrasil, l'albero cosmico che sostiene i nove mondi della mitologia norrena.",
...
const MIRA_NUM_UCCELLI = 3;
const MIRA_VELOCITA = 1.3;
const MIRA_TICK_MS = 40;
const MIRA_RAGGIO_COLPO = 7;

const MIRA_PREMI = [
  { soglia: 12, dracme: 200 },
  { soglia: 9, dracme: 120 },
  { soglia: 6, dracme: 70 },
  { soglia: 3, dracme: 35 },
  { soglia: 1, dracme: 15 }
...
function tickMira() {
  miraUccelli.forEach(u => {
    u.x += u.dx;
    u.y += u.dy;
    if (u.x <= 4 || u.x >= 96) u.dx *= -1;
    if (u.y <= 4 || u.y >= 96) u.dy *= -1;
...
function sparaMira(xPercento, yPercento) {
  if (!miraInPartita || miraFrecceRimaste <= 0) return;

  miraFrecceRimaste--;

  let colpito = false;
  for (const u of miraUccelli) {
    const dist = Math.hypot(u.x - xPercento, u.y - yPercento);
    if (dist <= MIRA_RAGGIO_COLPO) {
      colpito = true;
      miraPunteggio++;
      Object.assign(u, generaUccelloMira());
      break;
    }
  }

  miraUltimoColpo = { x: xPercento, y: yPercento, colpito };
  ...
  if (miraFrecceRimaste <= 0) {
    ...
    dracmeAttuali += premio;
    if (miraPunteggio >= 9) segnaFaticaCompletata("sonaglio");
    aggiornaTopbarProfilo();
    salvaProgressoCloud();
  }
...
function htmlSchermataMira() {
    ...
    return `
      <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; height:100%; gap:14px; padding:14px;">
        <div style="background:rgba(15,10,5,0.6); border-radius:10px; padding:14px; color:#e0d5c1; font-size:0.85rem; max-width:380px; text-align:center;">
          ...
          <p>Tocca lo schermo per scoccare una freccia nel punto esatto in cui miri: gli uccelli volano liberi e cambiano rotta in continuazione. Hai <b>${MIRA_FRECCE_MAX} frecce</b>, usale con precisione.</p>
        </div>
  ...
  const uccelliHTML = miraUccelli.map(u => `
    <div class="mira-uccello" style="left:${u.x}%; top:${u.y}%;">🕊️</div>
  `).join("");

  const colpoHTML = miraUltimoColpo ? `
    <div class="mira-flash ${miraUltimoColpo.colpito ? "mira-flash-colpito" : "mira-flash-mancato"}" style="left:${miraUltimoColpo.x}%; top:${miraUltimoColpo.y}%;">${miraUltimoColpo.colpito ? "✅" : "✖️"}</div>
  ` : "";
  ...
  return `
    <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; height:100%; gap:10px; padding:10px;">
      <div style="display:flex; justify-content:space-between; width:100%; max-width:400px; color:#e0d5c1; font-size:0.85rem;">
        <span>🏹 Frecce: <b style="color:#ffcc66;">${miraFrecceRimaste}</b></span>
        <span>Colpiti: <b style="color:#ffcc66;">${miraPunteggio}</b></span>
      </div>
      <div id="mira-campo" class="mira-campo">${uccelliHTML}${colpoHTML}</div>
    </div>`;
}
...
function collegaEventiMira() {
  ...
  if (campo) {
    campo.addEventListener("click", (e) => {
      ...
      const xPercento = ((e.clientX - rect.left) / rect.width) * 100;
      const yPercento = ((e.clientY - rect.top) / rect.height) * 100;
      sparaMira(xPercento, yPercento);
    });
  }
}
...
// ===== Serie Giornaliera: calendario a premi infinito, mai un reset se salti un giorno =====
...
function calcolaPremioSerie(giorno) {
  const dracme = Math.round(150 + 150 * Math.log(giorno));
  let frammenti = 0;
  if (giorno % 7 === 0) frammenti += 1;
  if (giorno % 30 === 0) frammenti += 2;
  return { dracme, frammenti };
}
...
function ritiraSerieGiornaliera() {
  ...
  const premio = calcolaPremioSerie(serieStato.giorni);
  dracmeAttuali += premio.dracme;
  if (premio.frammenti > 0) ambraAttuale += premio.frammenti;

  aggiornaTopbarProfilo();
  salvaProgressoCloud();
  sparaParticelle(premio.frammenti > 0 ? 5 : 3, document.getElementById("hub-btn-serie"));

  let testo = `🔥 Giorno ${serieStato.giorni}! +${premio.dracme} Dracme`;
  if (premio.frammenti > 0) testo += ` e +${premio.frammenti} Frammento${premio.frammenti > 1 ? "i" : ""} d'Ambra`;
  mostraToast(testo + "!");
...
const FURTO_TAPPE_TOTALI = 10;
const FURTO_INTERVALLO_MIN_MS = 900;
const FURTO_INTERVALLO_MAX_MS = 2400;

const FURTO_PREMI = [
  { soglia: 10, dracme: 220, frammenti: 1 },
  { soglia: 8, dracme: 130, frammenti: 0 },
  { soglia: 6, dracme: 80, frammenti: 0 },
  { soglia: 3, dracme: 30, frammenti: 0 },
...
function terminaFurto(catturato) {
  ...
  dracmeAttuali += premio.dracme;
  if (premio.frammenti > 0) ambraAttuale += premio.frammenti;
  if (furtoTappaAttuale >= 6) segnaFaticaCompletata("gregge");
...
function htmlSchermataFurto() {
  ...
  return `
    <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; height:100%; gap:16px; padding:14px;">
      <div style="color:#e0d5c1; font-size:0.85rem;">Passo <b style="color:#ffcc66;">${furtoTappaAttuale}</b> / ${FURTO_TAPPE_TOTALI}</div>
      <div class="furto-guardiano ${furtoGuardianoDistratto ? "furto-guardiano-distratto" : "furto-guardiano-attento"}">
        ${furtoGuardianoDistratto ? "🐕‍🦺" : "👀"}
      </div>
      <div style="color:${furtoGuardianoDistratto ? "#7ee787" : "#f56565"}; font-weight:bold; font-size:0.95rem;">
