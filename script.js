...
const PARTICELLE_LIVELLI = {
  1: { conteggio: 10, colori: ["#e0d5c1", "#c9a054"], velocita: 3.5, vitaMs: 700, shake: false },
  2: { conteggio: 18, colori: ["#7ee787", "#8bb8e0", "#e0d5c1"], velocita: 4.5, vitaMs: 850, shake: false },
  3: { conteggio: 28, colori: ["#8bb8e0", "#b794f6", "#e0d5c1"], velocita: 5.5, vitaMs: 1000, shake: false },
  4: { conteggio: 42, colori: ["#ffcc66", "#f6ad55", "#fff2cc"], velocita: 6.5, vitaMs: 1150, shake: true },
  5: { conteggio: 58, colori: ["#e09fae", "#ffcc66", "#f5f0ff"], velocita: 7.5, vitaMs: 1300, shake: true },
  6: { conteggio: 80, colori: ["#ffe9a8", "#ffcc66", "#f5f0ff", "#c9a054"], velocita: 9, vitaMs: 1550, shake: true }
};

// Spara un burst di particelle. Se xRel/yRel non sono passati, esplode dal centro dello schermo.
function sparaParticelle(livello, xRel, yRel) {
  const canvas = ottieniCanvasParticelle();
  const cfg = PARTICELLE_LIVELLI[Math.min(6, Math.max(1, livello || 1))];

  const cx = xRel !== undefined ? xRel : canvas.width / 2;
  const cy = yRel !== undefined ? yRel : canvas.height / 2;
...
function aggiornaTopbarProfilo() {
  ...
  const elLivello = document.getElementById("player-level");
  if (elLivello) elLivello.innerText = livelloGiocatore;
...
function renderContenutoFatiche() {
  ...
  if (sezioneFaticheCorrente === "scala") {
    ...
    document.getElementById("battle-title-outcome").innerText = "Le Stalle di Augia";
    content.innerHTML = bottoneIndietro + htmlSchermataAugia();
    collegaEventiAugia();
  } else if (sezioneFaticheCorrente === "cavalle") {
    document.getElementById("battle-title-outcome").innerText = "Le Cavalle Famigliche";
    content.innerHTML = bottoneIndietro + htmlSchermataCavalle();
    ...
    content.innerHTML = bottoneIndietro + htmlSchermataInseguimento();
    ...
    content.innerHTML = bottoneIndietro + htmlSchermataSonaglio();
...
function htmlHubFatiche() {
  return `
    <div style="display:flex; flex-direction:column; gap:12px; padding:20px; align-items:center; justify-content:flex-start; height:100%; overflow-y:auto;">
      <button type="button" id="hub-btn-scala" class="events-btn events-btn-main" style="width:100%; max-width:360px; padding:16px; font-size:1rem;">
        🦁 La Scala del Leone
      </button>
      <button type="button" id="hub-btn-ruota" class="events-btn events-btn-main" style="width:100%; max-width:360px; padding:16px; font-size:1rem;">
        🎡 Ruota della Fortuna
      </button>
      <button type="button" id="hub-btn-augia" class="events-btn events-btn-main" style="width:100%; max-width:360px; padding:16px; font-size:1rem;">
        🐄 Le Stalle di Augia
      </button>
      <button type="button" id="hub-btn-cavalle" class="events-btn events-btn-main" style="width:100%; max-width:360px; padding:16px; font-size:1rem;">
        🐴 Le Cavalle Famigliche
      </button>
      <button type="button" id="hub-btn-inseguimento" class="events-btn events-btn-main" style="width:100%; max-width:360px; padding:16px; font-size:1rem;">
        🦌 L'Inseguimento
      </button>
      <button type="button" id="hub-btn-sonaglio" class="events-btn events-btn-main" style="width:100%; max-width:360px; padding:16px; font-size:1rem;">
        🕊️ Il Sonaglio di Efesto
      </button>
...
function htmlGiocoAugia() {
  ...
  return `
    <div style="position:relative; display:flex; flex-direction:column; align-items:center; gap:8px; height:100%; padding:6px;">
      <div style="display:flex; justify-content:space-between; width:100%; max-width:600px; color:#e0d5c1; font-size:0.85rem;">
        <span>Mosse: <b style="color:#ffcc66;">${augiaMosseRimaste}</b></span>
        <span>Punteggio: <b style="color:#ffcc66;">${augiaPunteggio}</b></span>
      </div>
...
const ICONE_MITOLOGIA = {
  ...
  "Aborigena Australiana": "🪃", "Altre Tradizioni": "🌍"
};
...
function renderContenutoMythophedia() {
  ...
  if (!cultureMitoCorrente) {
    ...
    const tastiHtml = cultureOrdinate.map(cultura => {
      ...
      return `
        <button type="button" class="mito-tasto-cultura" data-cultura-tasto="${cultura}">
