    ...
    let cardHTML = `
      ...
      <div class="duel-card-bacheca ${classElite}">
        ...
        <div class="duel-card-header">
          ...
          <span> Sfidante: <strong>${sfida.creatore}</strong></span>
...
document.getElementById("btn-account")?.addEventListener("click", () => {
  ...
  authFirebase.signInWithRedirect(provider).catch((err) => {
    alert("Accesso non riuscito: " + err.message);
  });
});

authFirebase.getRedirectResult().then((result) => {
  if (result && result.user) {
    alert("DEBUG: getRedirectResult ha trovato l'utente " + result.user.email);
  } else {
    alert("DEBUG: getRedirectResult non ha trovato nessun utente (result vuoto)");
  }
}).catch((err) => {
  alert("DEBUG: getRedirectResult ha dato ERRORE: " + err.code + " - " + err.message);
});
...
authFirebase.onAuthStateChanged((user) => {
  ...
  if (!user) {
    alert("DEBUG: onAuthStateChanged dice che NON sei loggato");
    salvataggioCloudCaricato = false;
    return;
  }

  alert("DEBUG: onAuthStateChanged dice che SEI loggato come " + user.email);

  dbFirebase.ref("giocatori/" + user.uid).once("value").then((snapshot) => {
    if (snapshot.exists()) {
      applicaDatiCaricati(snapshot.val());
      alert("DEBUG: dati caricati dal database con successo");
    } else {
      salvaProgressoCloud();
      alert("DEBUG: nessun salvataggio precedente trovato, ne ho creato uno nuovo");
    }
    salvataggioCloudCaricato = true;
  }).catch((err) => {
    console.error("Errore caricamento cloud:", err);
    alert("DEBUG: errore leggendo il database: " + err.code + " - " + err.message);
  });
});
