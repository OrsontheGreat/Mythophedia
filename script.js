...
function generaGrigliaAugia() {
  const g = [];
  for (let r = 0; r < AUGIA_DIMENSIONE; r++) {
    g.push([]);
    for (let c = 0; c < AUGIA_DIMENSIONE; c++) {
      let simbolo;
      do {
...
function trovaMatchAugia(g) {
  ...
  for (let r = 0; r < AUGIA_DIMENSIONE; r++) {
    let run = 1;
    for (let c = 1; c <= AUGIA_DIMENSIONE; c++) {
      if (c < AUGIA_DIMENSIONE && g[r][c] === g[r][c - 1]) { run++; }
      else {
  ...
  for (let c = 0; c < AUGIA_DIMENSIONE; c++) {
    let run = 1;
    for (let r = 1; r <= AUGIA_DIMENSIONE; r++) {
      if (r < AUGIA_DIMENSIONE && g[r][c] === g[r - 1][c]) { run++; }
      else {
...
function risolviCascataAugia() {
  ...
  while (true) {
    ...
    for (let c = 0; c < AUGIA_DIMENSIONE; c++) {
      let colonna = [];
      for (let r = 0; r < AUGIA_DIMENSIONE; r++) if (augiaGriglia[r][c] !== null) colonna.push(augiaGriglia[r][c]);
      while (colonna.length < AUGIA_DIMENSIONE) colonna.unshift(Math.floor(Math.random() * AUGIA_SIMBOLI.length));
      for (let r = 0; r < AUGIA_DIMENSIONE; r++) augiaGriglia[r][c] = colonna[r];
    }
  }
  return puntiRound;
}
...
function htmlGiocoAugia() {
  ...
  for (let r = 0; r < AUGIA_DIMENSIONE; r++) {
    for (let c = 0; c < AUGIA_DIMENSIONE; c++) {
      ...
      celle += `<button type="button" class="augia-cella${selezionata ? " selezionata" : ""}" data-r="${r}" data-c="${c}">${AUGIA_SIMBOLI[augiaGriglia[r][c]]}</button>`;
    }
  }
  ...
  return `
    <div style="position:relative; display:flex; flex-direction:column; align-items:center; gap:8px; height:100%; padding:6px;">
      <div style="display:flex; justify-content:space-between; width:100%; max-width:420px; color:#e0d5c1; font-size:0.85rem;">
...
function collegaEventiAugia() {
  ...
  document.querySelectorAll(".augia-cella").forEach(cella => {
    cella.addEventListener("click", () => {
      ...
      augiaMosseRimaste--;
      augiaPunteggio += risolviCascataAugia();
