...
function htmlRuotaFortuna() {
  ...
  const legenda = SEGMENTI_RUOTA.map(s => `
    <div style="display:flex; align-items:center; gap:6px; font-size:0.68rem; color:#e0d5c1;">
      <span style="width:11px; height:11px; border-radius:3px; background:${s.colore}; display:inline-block; border:1px solid rgba(255,255,255,0.35); flex-shrink:0;"></span>${s.label}
    </div>`).join("");

  return `
    <div style="display:flex; flex-direction:column; align-items:center; padding:6px 14px 14px; gap:12px;">
      <div style="position:relative; width:210px; height:210px;">
        <div style="position:absolute; top:-16px; left:50%; transform:translateX(-50%); font-size:1.5rem; z-index:2; filter:drop-shadow(0 2px 3px rgba(0,0,0,0.9));">🔻</div>
        <div id="ruota-fortuna-disco" style="width:210px; height:210px; border-radius:50%; border:5px solid #c9a054; box-shadow:0 0 18px rgba(0,0,0,0.6), inset 0 0 18px rgba(0,0,0,0.4); background:${costruisciConicGradientRuota()}; transition: transform 4.2s cubic-bezier(0.15,0.68,0.1,1);"></div>
        <div style="position:absolute; top:50%; left:50%; transform:translate(-50%,-50%); width:30px; height:30px; border-radius:50%; background:radial-gradient(circle,#ffe9a8,#c9a054); border:3px solid #5c4d31; box-shadow:0 0 10px rgba(0,0,0,0.6); z-index:2;"></div>
      </div>
      <div style="display:grid; grid-template-columns:repeat(2,1fr); gap:4px 14px;">${legenda}</div>
      <div id="ruota-fortuna-esito" style="min-height:22px; text-align:center; color:#ffcc66; font-weight:bold; font-size:0.95rem;"></div>
