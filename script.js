...
function suonaClick() {
  suonaTono(700, 0.08, "sine", 0.28, 0, 500);
}

function suonaSfoglioPagina() {
  suonaRumoreStrappo(0.18, 0.22);
}

function suonaConferma() {
  suonaTono(600, 0.12, "sine", 0.32, 0, 900);
  suonaTono(900, 0.15, "sine", 0.22, 0.06);
}

// Ascoltatore globale: fa suonare un click su qualunque bottone del gioco,
// presente e futuro, senza dover toccare a mano ogni singolo tasto.
document.addEventListener("click", (e) => {
  const target = e.target.closest("button:not(:disabled), .map-tile, .fatica-tile, .mito-tasto-cultura");
  if (!target) return;
  if (target.id === "raccoglitore-pag-prec" || target.id === "raccoglitore-pag-succ") return;
  suonaClick();
});
