...
function miniImmagineCarta(carta, px) {
  ...
  return haImmagineFile(carta)
    ? `<img src="${carta.immagine}" alt="${carta.nome}" style="width:${px}px;height:${px}px;object-fit:cover;border-radius:5px;vertical-align:middle;">`
...
function mostraCartaFullscreen(carta) {
  ...
  const trattiHTML = tratti.length
    ? `<div class="fs-card-abilita-titolo">Abilità Speciali</div><div class="fs-card-traits">${tratti.map(t => `<span class="trait-badge">${ETICHETTE_TRATTI[t] || t}</span>`).join("")}</div>`
    : "";

  const immagineHTML = haImmagineFile(carta)
    ? `<img src="${carta.immagine}" alt="${carta.nome}" class="fs-card-img">`
...
const SOGLIE_XP = { 1: 50, 2: 200, 3: 500, 4: 1000, 5: 2000, 6: 999999 };
  ...
  const btnRaccoglitore = document.getElementById("btn-raccoglitore");

  if (btnRaccoglitore) {

    btnRaccoglitore.innerHTML = `<strong>📖 Il Raccoglitore</strong><br><span style="font-weight:normal; font-size:0.75rem;">${deckGiocatore.length} carte · C:${conteggi[1]} NC:${conteggi[2]} R:${conteggi[3]} E:${conteggi[4]} M:${conteggi[5]} L:${conteggi[6]}</span>`;
...
function renderizzaRaccoglitore(criterio) {
  ...
  modalTitle.innerText = `Il Raccoglitore (${deckGiocatore.length} carte)`;

  let carteFiltrate = ordinaCarteRaccoglitore(deckGiocatore, criterio);

 

  if(carteFiltrate.length === 0) {

    modalGrid.innerHTML = `<p style="color:#aaa; grid-column:span 5; text-align:center; padding-top:40px;">Non possiedi ancora nessuna carta.</p>`;
    ...
    carteFiltrate.forEach(carta => {
      ...
      const cardHTML = `
        ...
        <div class="creature-card ${carta.occupataInDifesa || carta.bloccataInDuello || pctVigore <= 0 ? 'occupata' : ''}">
          ...
          <div class="card-icon" id="card-icon-${carta.id}" style="cursor:pointer;">${haImmagineFile(carta) ? `<img src="${carta.immagine}" alt="${carta.nome}" class="card-icon-img">` : carta.immagine}</div>
          ...
          <button type="button" class="attack-btn" id="btn-vendi-${carta.id}" style="padding:5px; font-size:0.75rem; margin-top:4px; background:linear-gradient(to bottom, #742a2a, #4a1d1d); border-color:#5c2323;">Vendi (${carta.isJolly ? 10 : carta.livello * 15} 🪙)</button>
...
document.getElementById("btn-conferma-evoluzione").addEventListener("click", () => {
  ...
  document.getElementById("battle-report-content").innerHTML = `
    ...
    <p>La tua creatura <strong>${miniImmagineCarta(creaturaInEvoluzione, 28)} ${creaturaInEvoluzione.nome}</strong> è ascesa al grado di <strong>${creaturaInEvoluzione.stelle} ★</strong>!</p>
