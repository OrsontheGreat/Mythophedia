...
function renderizzaFiltrata(lvl, nomeLivello) {
  ...
  if(carteFiltrate.length === 0) {
    ...
    carteFiltrate.forEach(carta => {
      ...
      } else if (!carta.isJolly && carta.stelle < 8) {

        const elBtn = document.getElementById(`btn-evo-act-${carta.id}`);

        if (elBtn) elBtn.addEventListener("click", () => {

          alert(`DEBUG: pulsante non collegato per ${carta.nome}. bloccataInDuello=${carta.bloccataInDuello} occupataInDifesa=${carta.occupataInDifesa} vigore=${pctVigore}`);
...
function apriFinestraEvoluzione(carta) {
  ...
  document.getElementById("evolution-modal").classList.remove("hidden");
    ...
    alert("DEBUG errore in apriFinestraEvoluzione: " + err.message);
